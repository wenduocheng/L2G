...
def load_traffic(dataset, root_dir, batch_size, valid_split=-1):
    '''
    Traffic dataset of form (batch_size, 1, 512)
    '''
    if dataset not in ['ISCXVPN2016', 'USTC-TFC2016', 'CICIoT2022', 'ISCXTor2016']:
        raise ValueError(f"Unknown dataset: {dataset}")
    root = os.path.join(root_dir, f'{dataset}/tokenized_data/')

    train_dataset = torch.load(os.path.join(root, 'train_dataset.pt'))
    test_dataset = torch.load(os.path.join(root, 'test_dataset.pt'))

    if valid_split > 0:
        valid_dataset = torch.load(os.path.join(root, 'train_dataset.pt'))
        ...
        valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=batch_size, sampler=valid_sampler, num_workers=4, pin_memory=True)
    else:
