...
def main():
    ...
    metric, compare_metrics = get_metric(None, args.dataset)
        ...
        if not args.baseline and ((ep + 1) % retrain_freq == 0 or ep == epochs - 1):
            ...
            if prev_param_values is not None and torch.equal(param_values, prev_param_values):
                ...
                if os.path.isfile(os.path.join(args.save_dir, 'network_hps.npy')) or args.retrain_only:
                    ...
                    random_seed, lr, drop_rate, weight_decay, momentum = np.load(os.path.join(hp_dir, 'network_hps.npy'))
                    ...
                    #remove all saved weights except the selected one
                ...
                print("load from state space before HPO")
                retrain_model.load_state_dict(torch.load(os.path.join(init_dir, f'init_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}_before.pt')))
                ...
                # init_dir = args.hp_dir if args.retrain_only else args.save_dir
                # print("load from state space after HPO")
                # retrain_model.load_state_dict(torch.load(os.path.join(init_dir, f'init_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}_after.pt')))
                # retrain_optimizer = get_optimizer(momentum = momentum, weight_decay = weight_decay, type=optimizer_type)(retrain_model.parameters(), lr = lr)
                # retrain_model.set_arch_requires_grad(False)
                ...
                # print("\n------- Start Test without retraining --------")
                # test_model.load_state_dict(torch.load(os.path.join(init_dir, f'init_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}_after.pt')))
                # test_time_start = default_timer()
                # test_loss, test_score = evaluate(test_model, args.device, retrain_test_loader, loss, metric, retrain_n_test, decoder, transform, fsd_epoch=200 if args.dataset == 'FSD' else None)
                # test_time_end = default_timer()

                # print("[test init]", "\ttime elapsed:", "%.4f" % (test_time_end - test_time_start), "\ttest loss:", "%.4f" % test_loss, "\ttest score:", "%.4f" % test_score)


                np.save(os.path.join(args.save_dir, 'dash_final_results.npy'), {'ks':ks,'ds':ds,'lr':lr,'drop out':drop_rate,'weight decay':weight_decay,'momentum':momentum,'test best score':test_score})
                



def asha_inner_search(tune_config, arch_retrain, sample_shape, num_classes, config_kwargs, ks, ds, decoder, transform, args, optimizer_type, weight_sched_hpo, lr_sched_iter, loss, metric, retrain_clip, batch_size, n_train, compare_metrics):
    _, search_epochs, subsampling_ratio = get_hp_configs(args.dataset, n_train, arch_retrain)
    if args.split_state:
    ...
    torch.save(model_base.state_dict(), os.path.join(args.save_dir, f'init_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}_before.pt'))
    if compare_metrics == np.max:
        best_val_score = -100
    else:
        best_val_score = 100
    for retrain_ep in range(search_epochs):
        ...
        #save val_score to a txt file
        # with open(os.path.join(args.save_dir, f'val_score_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}.txt'), 'w') as f:
        #     f.write(f"{retrain_val_score}\n")
        if compare_metrics == np.max:
            if retrain_val_score > best_val_score:
                best_val_score = retrain_val_score
                torch.save(retrain_model.state_dict(), os.path.join(args.save_dir, f'init_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}_after.pt'))
        else:
            if retrain_val_score < best_val_score:
                best_val_score = retrain_val_score
                torch.save(retrain_model.state_dict(), os.path.join(args.save_dir, f'init_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}_after.pt'))
    ...
    # print("[hp search] random seed = ", random_seed, " lr = ", "%.6f" % lr, " drop rate = ", "%.2f" % drop_rate, " weight decay = ", "%.6f" % weight_decay, " momentum = ", "%.2f" % momentum, " time elapsed:", "%.4f" % (retrain_time_end - retrain_time_start), "\ttrain loss:", "%.4f" % retrain_loss, "\tval loss:", "%.4f" % retrain_val_loss, "\tval score:", "%.4f" % retrain_val_score)
    # torch.save(retrain_model.state_dict(), os.path.join(args.save_dir, f'init_{random_seed}_{lr}_{drop_rate}_{weight_decay}_{momentum}_after.pt'))
    del retrain_model
...
def initialize_asha_search(arch_retrain, sample_shape, num_classes, config_kwargs, ks, ds, decoder, transform, args, optimizer_type, weight_sched_hpo, lr_sched_iter, loss, metric, retrain_clip, compare_metrics, batch_size, n_train):
    search_program = partial(asha_inner_search, arch_retrain=arch_retrain, sample_shape=sample_shape, num_classes=num_classes, config_kwargs=config_kwargs, ks=ks, ds=ds, decoder=decoder, transform=transform, args=args, optimizer_type=optimizer_type, weight_sched_hpo=weight_sched_hpo, lr_sched_iter=lr_sched_iter, loss=loss, metric=metric, retrain_clip=retrain_clip, batch_size=batch_size, n_train=n_train, compare_metrics=compare_metrics)
    search_space = {
        "random_seed": tune.randint(0, 500),
        "lr": tune.loguniform(1e-5, 5e-1),
        # "lr": tune.loguniform(8e-3, 5e-1),
        "drop_rate": tune.choice([0, 0.05, 0.1]),
        "weight_decay": tune.loguniform(5e-7, 5e-3),
        "momentum": tune.uniform(0.9, 1)
    }
    # search_space = {
    #     "random_seed": tune.grid_search([0, 1]),
    #     "lr": tune.grid_search([0.1, 0.01, 0.001, 0.0001]),
    #     "drop_rate": tune.grid_search([0, 0.05]),
    #     "weight_decay": tune.grid_search([5e-4, 5e-6]),
    ...
    print("mode:", mode)
    main_with_gpu = tune.with_resources(search_program, {"cpu": 10, "gpu": 1})
    ...
    #     tune_config=tune.TuneConfig(
    #         num_samples=150,
    #         scheduler=HyperBandScheduler(metric="val_score", mode=mode, max_t=40),
    #     ),
    #     param_space=search_space,
    # )
