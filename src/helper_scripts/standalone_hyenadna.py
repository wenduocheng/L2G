...
class HyenaFilter(OptimModule):
    def __init__(
        ...
        modules = [] #
        modules.append(nn.Linear(emb_dim, order))
        modules.append(act)
        
        for i in range(num_inner_mlps):
            modules.append(nn.Linear(order, order))
            modules.append(act)

        modules.append(nn.Linear(order, d_model, bias=False))
        self.implicit_filter = nn.Sequential(*modules)  #
...
def create_mlp_cls(d_model, d_inner=None, device=None, dtype=None):
    ...
    mlp_cls = partial(Mlp, hidden_features=inner_dim,
                          activation=partial(F.gelu), **factory_kwargs)
...
class HyenaDNAModel(nn.Module):
    ...
    def forward(self, input_ids, position_ids=None, state=None, return_embedding=False): # state for the repo interface
